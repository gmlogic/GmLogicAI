...
Public Class Form1
    ...
    Private Function ExtractParagraphTitles(cleanText As String) As List(Of String)
        ...
        While i < lines.Length
            If IsSeparatorLine(lines(i)) Then
                i += 1
                Continue While
            End If
            ...
            While i < lines.Length AndAlso Not IsSeparatorLine(lines(i))
                Dim stripped = Regex.Replace(lines(i).Trim(), "^[^\p{Lu}]*", String.Empty)
            ...
            Dim prevIsSeparator = (blockStart = 0) OrElse IsSeparatorLine(lines(blockStart - 1))
            Dim nextIsSeparator = (i >= lines.Length) OrElse IsSeparatorLine(lines(i))

            If Not (prevIsSeparator AndAlso nextIsSeparator) Then
                Continue While
            End If
            ...
            ' OCR often splits one heading into 2 lines. Test both single line and merged variants.
            Dim candidates As New List(Of String)()
            For Each headingLine In blockLines
                candidates.Add(headingLine)
            Next

            If blockLines.Count >= 2 Then
                candidates.Add(String.Join(" ", blockLines))
                candidates.Add(String.Join(" ", blockLines.Take(2)))
            End If

            For Each candidate In candidates
                If IsLikelyHeadingText(candidate) Then
